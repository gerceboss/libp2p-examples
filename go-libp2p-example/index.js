...
async function connectWithTransports (mode = 'webrtc') {
  ...
  try {
    ...
    try {
      ...
      if (DEBUG) {
        console.log('Bootstrap addresses:', bootstrapAddresses)
      }
    } catch (err) {
      log(`⚠️ Failed to fetch relay addresses: ${err.message}`, true)
      log(`Trying fallback addresses from bootstrappers.js...`)
      ...
      if (DEBUG) {
        console.log('Fallback bootstrap addresses:', bootstrapAddresses)
      }
    }
    ...
    // Log pubsub topics for debugging
    if (DEBUG) {
      setTimeout(() => {
        const topics = libp2pNode.services.pubsub.getTopics()
        console.log('[Debug] Subscribed topics:', topics)
        console.log('[Debug] Discovery topic subscribers:', 
          libp2pNode.services.pubsub.getSubscribers('_peer-discovery._p2p._pubsub').map(p => p.toString()))
      }, 2000)
    }
    ...
    libp2pNode.addEventListener('peer:discovery', async (evt) => {
      ...
      const peerIdShort = peerId.toString().slice(0, 8) + '...' + peerId.toString().slice(-4)

      // Don't connect to ourselves
      if (libp2pNode.peerId.equals(peerId)) {
        return
      }

      if (libp2pNode.getConnections(peerId).length > 0) {
        if (DEBUG) {
          log(`Already connected to discovered peer: ${peerIdShort}`)
        }
        return
      }

      log(`🔍 Discovered peer: ${peerIdShort}, attempting to connect...`)
      try {
        await libp2pNode.dial(peerId)
        log(`✅ Successfully connected to peer: ${peerIdShort}`)
      } catch (err) {
        log(`❌ Failed to dial peer ${peerIdShort}: ${err.message}`, true)
        if (DEBUG) {
          console.error('Dial error details:', err)
        }
      }
    })

    // Also listen for peer:discovery events and log them
    libp2pNode.addEventListener('peer:discovery', (evt) => {
      if (DEBUG) {
        const peerId = evt.detail.id.toString()
        console.log('[Peer Discovery] Discovered:', peerId.slice(0, 8) + '...' + peerId.slice(-4))
      }
    })
    ...
    const updateInterval = setInterval(() => {
      ...
      // Debug: Log peer count periodically
      if (DEBUG) {
        const connections = libp2pNode.getConnections()
        const peerCount = new Set(connections.map(c => c.remotePeer.toString())).size
        if (peerCount > 0) {
          console.log(`[Periodic Check] Connected to ${peerCount} peer(s)`)
        }
      }
